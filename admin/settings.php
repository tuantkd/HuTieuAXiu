...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    admin_flash_set('Đã lưu cài đặt cửa hàng.', 'success');
...
<div class="panel">
    ...
    <form method="post" class="form-card">
        <div class="field">
            ...
            <input id="shop_name" type="text" name="shop_name" value="<?= admin_h($setting['shop_name']) ?>" required>
        </div>
        <div class="field">
            <label for="phone">Số điện thoại</label>
            <input id="phone" type="text" name="phone" value="<?= admin_h($setting['phone']) ?>">
        </div>
        <div class="field field--full">
            <label for="address">Địa chỉ</label>
            <input id="address" type="text" name="address" value="<?= admin_h($setting['address']) ?>">
        </div>
        <div class="field">
            <label for="open_hours">Giờ mở cửa</label>
            <input id="open_hours" type="text" name="open_hours" value="<?= admin_h($setting['open_hours']) ?>" placeholder="06:00 - 21:30">
        </div>
        <div class="field">
            <label for="logo_url">Logo / ảnh đại diện</label>
            <input id="logo_url" type="text" name="logo_url" value="<?= admin_h($setting['logo_url']) ?>" placeholder="assets/img/logo-placeholder.png">
        </div>
        <div class="field field--full">
