...
<div class="section-title">Chọn nhóm món</div>
<div class="tabs"><a class="tab <?=$filter=='all'?'active':''?>" href="index.php">Tất cả</a><a
        class="tab <?=$filter=='mon-an'?'active':''?>" href="?cat=mon-an">Món ăn</a><a
        class="tab <?=$filter=='nuoc-uong'?'active':''?>" href="?cat=nuoc-uong">Nước uống</a></div>
<div class="section-title">Chọn món</div>
<div class="grid <?=$filter=='nuoc-uong'?'drink':''?>">
    <?php while($p=$products->fetch_assoc()): ?><form method="post" class="product"><img src="<?=h($p['image_url'])?>">
        <div class="p"><b><?=h($p['name'])?></b>
            <div class="price"><?=money_vnd($p['price'])?></div><input type="hidden" name="add_product_id"
                value="<?=$p['id']?>"><input type="hidden" name="quantity" value="1"><button class="btn btn-light full"
                style="margin-top:8px">+ Thêm</button>
        </div>
...
<div class="summary-card">
    ...
    <div class="mini-grid">
        <div class="mini"><span class="emoji">🍜</span>
            <div>Món ăn<br><b><?=$food?:0?> tô/phần</b></div>
        </div>
        <div class="mini"><span class="emoji">🥤</span>
            <div>Nước uống<br><b><?=$drink?:0?> ly/chai</b></div>
        </div>
        <div class="mini"><span class="emoji">🪑</span>
            <div>Ăn tại quán<br><b><?=(int)$sum['dine_in']?> đơn</b></div>
        </div>
        <div class="mini"><span class="emoji">🛍️</span>
            <div>Mang đi<br><b><?=(int)$sum['takeaway']?> đơn</b></div>
        </div>
    </div>
